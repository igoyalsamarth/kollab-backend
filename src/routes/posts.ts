...
posts.post('/populate_static_posts', async (req: Request, res: Response) => {
    try {
        ...
        const createPosts = staticPosts.map((post: any) => prisma.staticPostsDetails.create({
            data: {
                id: post.id,
...
posts.post('/populate_reels', async (req: Request, res: Response) => {
    try {
        ...
        await prisma.$connect();
        const createReels: any[] = reelsPosts.map((post: any) => prisma.reelPostsDetails.create({
            data: {
                id: post.id,
                instaAccount: req.body.instaAccount,
                imgSource: post.byteaImage,
